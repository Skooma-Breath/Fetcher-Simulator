...
if ($IncludeTesterTools) {
    ...
    if (-not (Test-Path -LiteralPath $releaseRootFiles -PathType Container)) {
        throw "Fetcher tester tools directory was not found: $releaseRootFiles"
    }
...
Set-Content -LiteralPath $readmePath -Encoding ASCII -Value @(
    ...
    $(if ($IncludeTesterTools) {
        ...
        "This clean package intentionally excludes Fetcher tester tools and bundled gameplay mods. To opt in, download Join-Fetcher-Test-Channel.bat from https://github.com/Skooma-Breath/Fetcher-Simulator/releases/tag/fetcher-tester-tools."
    }),
    "",
