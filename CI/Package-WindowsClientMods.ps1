...
function Convert-ToUserConfigPath {
    param(
        [Parameter(Mandatory = $true)]
        [string] $Path
    )
...
$userOpenmwCfg = Join-Path $userConfigPath "openmw.cfg"

$configLines = New-Object System.Collections.Generic.List[string]
$configLines.Add("# Portable tester config generated by CI.")
$configLines.Add("# This file intentionally ignores Documents\My Games\OpenMW and delegates content to userdata\openmw.cfg.")
...
$userConfigLines = New-Object System.Collections.Generic.List[string]
$userConfigLines.Add("# Portable tester user config generated by CI.")
$userConfigLines.Add("# The wizard and launcher own this file.")
$userConfigLines.Add("data=../resources/vfs-mw")
foreach ($dataDir in @($manifest.dataDirs)) {
    if ($dataDir) {
        $userConfigLines.Add("data=$(Convert-ToUserConfigPath -Path $dataDir)")
    }
}
foreach ($archive in @($manifest.fallbackArchives)) {
    if ($archive) {
        $userConfigLines.Add("fallback-archive=$archive")
    }
}
foreach ($content in @($manifest.content)) {
    if ($content) {
        $userConfigLines.Add("content=$content")
    }
}
$userConfigLines.Add("")
Set-Content -LiteralPath $userOpenmwCfg -Value $userConfigLines -Encoding ASCII
...
Set-Content -LiteralPath $readmePath -Encoding ASCII -Value @(
    ...
    "Load order and content entries are stored in .\userdata\openmw.cfg so the wizard and launcher can update them cleanly.",
...
Write-Output "Portable OpenMW tester user config written to $userOpenmwCfg."
